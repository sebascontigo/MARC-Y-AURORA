...
THEMES = {
    "marc": dict(
        ...
        ink=(242, 238, 230),
        dim=(200, 195, 185),
        brand="D E S P I E R T A",
        author="MARC SOUZA",
        handle="@marcsouza.7",
        outdir=os.path.join(HERE, "HISTORIAS"),
    ),
    "aurora": dict(
        top=(244, 248, 242),
        bottom=(222, 235, 222),
        ...
        ink=(46, 93, 63),
        dim=(110, 140, 115),
        brand="S O B E R A N Í A   V I T A L",
        author="AURORA VELAV",
        handle="@auroravelav",
        outdir=os.path.join(HERE, "..", "..", "AURORA", "HISTORIAS"),
    ),
}
...
def main():
    ...
    ap.add_argument(
        "--fecha", default=None, help="YYYY-MM-DD para planificar dias futuros"
    )
    a = ap.parse_args()
    T = THEMES[a.cliente]
    W, H = 1080, 1920
    ...
    for y in range(H):
        ...
        d0.line(
            [(0, y), (W, y)],
            fill=tuple(
                int(T["top"][i] + (T["bottom"][i] - T["top"][i]) * t) for i in range(3)
            ),
        )
    ...
    img.paste(
        Image.new("RGB", (W, H), T["glow"]),
        (0, 0),
        ov.filter(ImageFilter.GaussianBlur(140)),
    )
    ...
    d.text(
        ((W - d.textlength(T["brand"], font=f_small)) / 2, 150),
        T["brand"],
        font=f_small,
        fill=T["accent"],
    )
    d.text(
        ((W - d.textlength(T["author"], font=ImageFont.truetype(F_BOLD, 26))) / 2, 200),
        T["author"],
        font=ImageFont.truetype(F_BOLD, 26),
        fill=T["dim"],
    )
    ...
    for ln in hl:
        d.text(
            ((W - d.textlength(ln, font=f_hook)) / 2, y), ln, font=f_hook, fill=T["ink"]
        )
        y += 100
    # texto medio
    if a.texto:
        ...
        for ln in wrap(d, a.texto, f_txt, W - 220):
            d.text(
                ((W - d.textlength(ln, font=f_txt)) / 2, y),
                ln,
                font=f_txt,
                fill=T["dim"],
            )
    ...
    by0 = H - 380
    d.rounded_rectangle(
        [bx0, by0, bx1, by0 + box_h], radius=24, outline=T["accent"], width=4
    )
    cl = wrap(d, a.cta, f_cta, (bx1 - bx0) - 80)
    cy = by0 + (box_h - len(cl) * 54) / 2 + 8
    for ln in cl:
        d.text(
            ((W - d.textlength(ln, font=f_cta)) / 2, cy), ln, font=f_cta, fill=T["ink"]
        )
        cy += 54
    d.text(
        ((W - d.textlength(T["handle"], font=f_small)) / 2, H - 170),
        T["handle"],
        font=f_small,
        fill=T["accent"],
    )
    day = (
        (ap.fecha if ap.fecha else datetime.date.today().isoformat())
        if False
        else (a.fecha if hasattr(a, "fecha") else None)
    ...
    # caption sugerido
    with open(
        os.path.join(out, f"CAPTION_{a.cliente}.txt"), "w", encoding="utf-8"
    ) as f:
        f.write(
            a.hook
            + "\n\n"
            ...
            + "\n\n#"
            + (
                "despierta #marcsouza #mente #transformacion"
                if a.cliente == "marc"
                else "soberaniavital #auroravelav #habitos #salud"
            )
        )
